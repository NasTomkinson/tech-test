...
const appbarItems: AppbarItemConfig[] = [
...
const AppbarItem = ({ icon, label, link, active }: { icon: string; label: string, link: string, active: boolean }) => {
    return (
        <Link href={link} aria-current={active ? "page" : undefined} className={`flex min-w-0 flex-1 flex-col items-center justify-center h-full md:h-auto md:flex-row md:gap-2 p-2 md:w-full md:flex-none md:justify-start rounded ${active ? 'text-primary bg-primary-light/30' : 'text-gray-500'}`}>
